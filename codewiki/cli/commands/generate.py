...
def generate_command(
    ...
    try:
        ...
        if api_key is None:
            raise ConfigurationError("API key is missing. Please run 'codewiki config validate'")
        ...
        agent_instructions_dict: dict[str, Any] | None = None
        persistent_instructions = config.agent_instructions
        if runtime_instructions and not runtime_instructions.is_empty():
            ...
            merged = AgentInstructions(
                ...
                or persistent_instructions.include_patterns,
                exclude_patterns=runtime_instructions.exclude_patterns
                or persistent_instructions.exclude_patterns,
                focus_modules=runtime_instructions.focus_modules
                or persistent_instructions.focus_modules,
                doc_type=runtime_instructions.doc_type or persistent_instructions.doc_type,
                custom_instructions=runtime_instructions.custom_instructions
                or persistent_instructions.custom_instructions,
            )
            agent_instructions_dict = merged.to_dict()
        elif not persistent_instructions.is_empty():
