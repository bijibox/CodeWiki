...
def config_set(
    ...
    try:
        ...
        validated_data: dict[str, str | int] = {}
        ...
        api_key_value = validated_data.get("api_key")
        validated_api_key: str | None = api_key_value if isinstance(api_key_value, str) else None

        base_url_value = validated_data.get("base_url")
        validated_base_url: str | None = base_url_value if isinstance(base_url_value, str) else None

        main_model_value = validated_data.get("main_model")
        validated_main_model: str | None = (
            main_model_value if isinstance(main_model_value, str) else None
        )

        cluster_model_value = validated_data.get("cluster_model")
        validated_cluster_model: str | None = (
            cluster_model_value if isinstance(cluster_model_value, str) else None
        )

        fallback_model_value = validated_data.get("fallback_model")
        validated_fallback_model: str | None = (
        ...
        max_token_per_module_value = validated_data.get("max_token_per_module")
        validated_max_token_per_module: int | None = (
            max_token_per_module_value if isinstance(max_token_per_module_value, int) else None
        )

        max_token_per_leaf_module_value = validated_data.get("max_token_per_leaf_module")
        validated_max_token_per_leaf_module: int | None = (
            max_token_per_leaf_module_value
            if isinstance(max_token_per_leaf_module_value, int)
            else None
        )

        max_depth_value = validated_data.get("max_depth")
        validated_max_depth: int | None = (
            max_depth_value if isinstance(max_depth_value, int) else None
        )
